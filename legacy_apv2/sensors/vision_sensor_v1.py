...
class VisionSensorV1:
    ...
    def _build_contour_bundle(self, *, image: Image.Image, source_type: str) -> dict[str, Any]:
        ...
        for contour_index, contour in enumerate(top_contours[:6]):
            ...
            component_edge_mask = np.zeros((height, width), dtype=np.uint8)
            cv2.drawContours(component_edge_mask, [contour], -1, 255, thickness=2)
            component_outline_rgba = np.zeros((height, width, 4), dtype=np.uint8)
            component_outline_rgba[component_edge_mask > 0, 0] = 255
            component_outline_rgba[component_edge_mask > 0, 1] = 247
            component_outline_rgba[component_edge_mask > 0, 2] = 236
            component_outline_rgba[component_edge_mask > 0, 3] = 255
            component_silhouette_rgba = np.zeros((height, width, 4), dtype=np.uint8)
            component_silhouette_rgba[:, :, :3] = np.where(
                component_mask[:, :, None] > 0,
                np.array([224, 236, 248], dtype=np.uint8),
                np.zeros((1, 1, 3), dtype=np.uint8),
            )
            component_silhouette_rgba[:, :, 3] = np.where(component_mask > 0, 168, 0).astype(np.uint8)
            component_silhouette_rgba[component_edge_mask > 0, 0] = 255
            component_silhouette_rgba[component_edge_mask > 0, 1] = 248
            component_silhouette_rgba[component_edge_mask > 0, 2] = 236
            component_silhouette_rgba[component_edge_mask > 0, 3] = 255
            component_foreground_rgba = np.zeros((height, width, 4), dtype=np.uint8)
            component_foreground_rgba[:, :, :3] = np.where(
                component_mask[:, :, None] > 0,
                (rgb.astype(np.float32) * 0.95).astype(np.uint8),
                np.zeros((1, 1, 3), dtype=np.uint8),
            )
            component_foreground_rgba[:, :, 3] = component_mask
            component_color_edges = cv2.bitwise_and(color_edges, component_mask)
            component_luma_edges = cv2.bitwise_and(edges, component_mask)
            component_composite_rgba = np.zeros((height, width, 4), dtype=np.uint8)
            component_composite_rgba[:, :, :3] = np.where(
                component_mask[:, :, None] > 0,
                np.clip(rgb.astype(np.float32) * 0.92 + 10.0, 0, 255).astype(np.uint8),
                np.zeros((1, 1, 3), dtype=np.uint8),
            )
            component_composite_rgba[:, :, 3] = np.where(component_mask > 0, 176, 0).astype(np.uint8)
            component_composite_rgba[component_color_edges > 0, :3] = np.clip(
                rgb[component_color_edges > 0].astype(np.float32) * 1.05,
                0,
                255,
            ).astype(np.uint8)
            component_composite_rgba[component_color_edges > 0, 3] = 255
            component_composite_rgba[component_luma_edges > 0, 0] = 255
            component_composite_rgba[component_luma_edges > 0, 1] = 248
            component_composite_rgba[component_luma_edges > 0, 2] = 236
            component_composite_rgba[component_luma_edges > 0, 3] = 255
            component_composite_rgba[component_edge_mask > 0, 0] = 255
            component_composite_rgba[component_edge_mask > 0, 1] = 252
            component_composite_rgba[component_edge_mask > 0, 2] = 242
            component_composite_rgba[component_edge_mask > 0, 3] = 255
            component_assets = {
                "mask_data_url": self._encode_png_data_url(component_mask, mode="L"),
                "outline_data_url": self._encode_png_data_url(component_outline_rgba, mode="RGBA"),
                "silhouette_data_url": self._encode_png_data_url(component_silhouette_rgba, mode="RGBA"),
                "foreground_data_url": self._encode_png_data_url(component_foreground_rgba, mode="RGBA"),
                "composite_data_url": self._encode_png_data_url(component_composite_rgba, mode="RGBA"),
            }
        ...
        *,
        ...
        if contour_components:
            ...
            for index, component in enumerate(contour_components[:3]):
                ...
                component_assets = dict(component.get("component_assets", {}) or {})
                if component_assets:
                    attrs["component_assets"] = component_assets
                samples.append(
                    {
